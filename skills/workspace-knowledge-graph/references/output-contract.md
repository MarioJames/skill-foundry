...
- `MEMORY.md`：按需消费的非权威工作上下文，只保存无法由代码、git/PR/CI、任务系统或图谱事实源可靠恢复，且会改变后续行为的信息。文件只放实际条目，无条目时仅保留标题和空占位；写入与消费规则属于 `AGENTS.md`，不在这里复制。以 `workspace` / `repo:<repo>` 为父 scope，具体任务嵌套 `task:<task-key>`；允许类型、写入门槛、紧凑格式、晋升/压缩和冲突处理见 [memory-protocol.md](memory-protocol.md)。当前用户指令和当前事实证据优先于历史记忆；记忆不能授权重复执行外部操作。
...
对**内容纪律问题只警告**：业务域/共享文档的摘要与命名、表格形态与证据单元格、链接标签、易漂移值（行号锚点、字面数量）、读者字段语义、零命中代码标识符、未决排查措辞、MEMORY 占位共存/旧格式/明显图谱流水账、子区覆盖缺口、孤儿或单数目录等。
...
9. `.workspace/state/discovery.json`：可自动刷新的扫描快照，用于审计或排障
