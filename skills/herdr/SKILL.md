---
name: herdr
description: 在会话开始或主任务变化时命名当前 Herdr 标签页，包括只读问答和无并行工作的评估，无需显式提及 Herdr。也用于有收益的并行工作、Jev 多任务调度与思考力度选择，以及 Herdr Agent 和终端管理。
---
...
For useful parallel work, read [Jev task scheduling](references/jev-scheduling.md). Define revisioned tasks, dependencies, acceptance and known resource ownership. One `~/.config/herdr/agents.json` (or explicit `--config`) maps `ordinary / moderate / complex` to native engine/model/reasoning profiles; use [the example](examples/agents.json). Keep ordinary single-owner work direct.

[`decide-tasks.ts`](scripts/decide-tasks.ts) classifies eligible tasks through Jev, maps profiles locally, then asks Jev to choose a bounded dispatch wave. Optional config `manual_override` forces every dispatched task onto a named route (for example `{"route":"ordinary"}`) or an explicit engine/model/reasoning profile; null or omission restores automatic routing. Honor this user override before difficulty-based profile selection, including when delegating directly; it does not change classification or bypass dispatch guards. Classification does not change to escape engine availability. Unknown capabilities, missing ownership, low confidence or API errors return to the main Agent; no silent model substitution. Probe is read-only and `supported` means the requested profile passes current model-catalog and adapter-syntax checks, not a tested launch or live authorization. Model changes belong in `agents.json`; do not add model/version/effort allowlists to the skill.

[`dispatch-tasks.ts`](scripts/dispatch-tasks.ts) rechecks a selected decision and atomically reserves its wave in the same private state file before creating lanes, starting Agents and submitting work. Persist each effect separately. Unknown outcomes retain reservations and require observation or owner reconciliation; never replay a timed-out start or prompt. Revisions cannot overlap unresolved work on the same logical task. Accept delivered artifacts before unlocking dependencies, then decide again. The main Agent retains integration, final verification, and cleanup of owned lanes.
