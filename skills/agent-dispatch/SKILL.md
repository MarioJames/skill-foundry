...
2. **把盘点结果交给 Jev。** 当前工作作为并行背景，未推进工作作为待判断项；即使新增指令只是修正，也重新看积压工作是否能推进。由 Jev 判断语义独立性、并行收益和组合，不让父任务先以“同项目”“同页面”“任务小”或“自己做更快”筛掉待办。单项和批次都使用 [统一 CLI](references/cli.md) 的公共 `tasks` 输入；不要只发送父任务已经选中的一项。
