...
`supported` is **catalog_only** evidence permitting a launch attempt, not proof that a session has launched or that the account has quota. Missing, stale, version-mismatched or malformed metadata stays `unknown`; an explicitly unsupported effort or an absent Qoder model is `unsupported`. Refresh Codex's model catalog through the CLI when needed, then decide again; do not bypass a failed probe or substitute another model. Actual launch failures follow the normal dispatch failure/unknown-outcome handling, with no automatic retry or fallback. No probe runs a model prompt.

Each field's readback remains null unless separately observed. Probe does not guarantee account quota, cwd tools or task-specific capabilities. V1 blocks nonempty task capability requirements until the owner resolves them in decomposition; it does not invent a generic tool-capability registry.
