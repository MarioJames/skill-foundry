...
先按 `SKILL.md` 盘点未完成工作；这一步不需要创建 JSON 或状态文件。找到就绪的独立候选后才进入下面的交接流程，由 Jev 判断是否值得并行。
...
> 父任务首次收到多项需求，以及收到追加/变更指令、拆出独立工作或收到子任务结果时，先加载并执行 agent-dispatch 的轻量判断，再继续编辑或等待。盘点全部未完成工作，不能仅因同项目、同页面或同目标归入串行；有输入齐备、读写范围可隔离的独立候选时交给 Jev 判断，没有候选则说明具体原因。受委派工作者不递归派发。
