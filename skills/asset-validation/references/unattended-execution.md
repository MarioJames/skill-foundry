...
1. Read only the asset entry files needed to classify type and purpose — `SKILL.md`, `plugin.json` (the only pre-registration entry file for plugins), agent frontmatter, or the rule matcher.
...
Everything else waits until after that first write: bundled `skills/`, `agents/`, hooks, or marketplace files; rig source, environment checks, or plugin-internal validation; this skill's `references/`, `scripts/acc.py`, `scripts/observe.py`; and any discovery such as `python3 "$ACC" --help`, `python3 "$ACC" asset --help`, `ls "$ACC"`, `echo $ACCEPTANCE_TMPDIR`, phase headings, or scanning this skill directory. The command contract in this reference is authoritative; do not open references before the first `acc` write.
...
if [ -n "${ACCEPTANCE_SANDBOX:-}" ]; then
  ACC="$(find "$ACCEPTANCE_SANDBOX/.iso" -path '*/skills/asset-validation/scripts/acc.py' -type f 2>/dev/null | head -1)"
fi
if [ -z "$ACC" ] && [ -f "skills/asset-validation/scripts/acc.py" ]; then
  ACC="skills/asset-validation/scripts/acc.py"
fi
if [ -z "$ACC" ]; then
  ACC="<loaded-skill-dir>/scripts/acc.py"
fi
test -f "$ACC" || { echo "asset-validation acc.py not found" >&2; exit 2; }
python3 "$ACC" bootstrap --name <asset_name> --type <type> --source <source_path> --goal "<one-line user goal>"
```
...
never use a fixed path such as `/tmp/acc-toy`, `/tmp/acc-work-path.txt`, or any fixed `/tmp/.<name>_marker`. If the path must be persisted, write it to `"$WORK/.workpath"` or another file inside the current round sandbox / `ACCEPTANCE_TMPDIR`.
...
After updating acceptance artifacts, prefer the typed profile runner when it supports the asset type: `acc profile run-task --acceptance <acceptance_id> --task t1 --mode <mode> --cli <claude|codex>`. It wraps start/launch/feed/bounded wait/capture/record with the asset-type staging rules and secret redaction. If the type profile is not implemented yet, or when debugging an individual stage, then run `acc start --acceptance <acceptance_id> --mode <mode> --cli <claude|codex>`, `acc launch --round <round_id> --cli <claude|codex>`, and `acc feed-task --round <round_id> --task t1`. In unattended mode, use `claude` unless the user explicitly selected `codex`.

Do not run `acc ... --help` or `acc record -h` during unattended execution; the command contract here is authoritative. Store raw ids in files when needed, never `KEY=value` lines unless the later command explicitly strips the prefix. Shell variables do not persist across separate Bash tool calls; resolve `ACC` in each batch or store the path in a file inside `"$WORK"` such as `"$WORK/.acc-path"`.
...
Independent verification still goes through `acc` read commands such as `acc round list`, `acc history`, and `acc show`. Do not run `sqlite3` on `state.sqlite3`; if a read is missing, add a narrow `acc` read command first.
