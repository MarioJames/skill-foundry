"""SQLite facts for the Agents Orchestrator Runtime schema."""
...
RUNTIME_HOME_ENV = compat_env.legacy_name("HOME")
RUNTIME_HOME_DIRECTORY = compat_env.RUNTIME_HOME_DIRECTORY
SCHEMA_VERSION = 3
...
def runtime_root():
    return compat_env.runtime_root()


def db_path():
    return compat_env.db_path()
...
def initialize_schema():
    ...
    try:
        ...
        if 2 not in applied:
            con.executescript(MIGRATION_2_SQL)
            con.execute(
            ...
            applied.add(2)
        if 3 not in applied:
            con.executescript(MIGRATION_3_SQL)
            con.execute(
...
def get_mode(mode_id, con=None):
    return _one("SELECT * FROM modes WHERE mode_id = ?", (mode_id,), con)


def get_mode_task(task_id, con=None):
    return _one(
...
def inspect_modes(root_id, mode_id=None, con=None):
    params = [root_id]
    predicate = ""
    if mode_id is not None:
        predicate = " AND mode_id=?"
        params.append(int(mode_id))
    modes = fetchall(
        "SELECT * FROM modes WHERE root_id=?" + predicate + " ORDER BY mode_id",
        tuple(params),
        con,
    )
    result = []
    for mode in modes:
        ...
        item["rounds"] = fetchall(
            "SELECT * FROM mode_rounds WHERE mode_id=? ORDER BY round_no",
            (mode["mode_id"],),
            con,
        )
        item["tasks"] = fetchall(
        ...
        item["findings"] = fetchall(
            "SELECT * FROM mode_findings WHERE mode_id=? ORDER BY finding_id",
            (mode["mode_id"],),
            con,
        )
        finding_ids = [row["finding_id"] for row in item["findings"]]
        if finding_ids:
            marks = ",".join("?" for _ in finding_ids)
            item["provenance"] = fetchall(
                """SELECT * FROM mode_finding_provenance
                   WHERE finding_id IN (%s) ORDER BY provenance_id""" % marks,
                tuple(finding_ids),
                con,
            )
            item["verifications"] = fetchall(
                """SELECT * FROM mode_verifications
                   WHERE finding_id IN (%s) ORDER BY verification_id""" % marks,
                tuple(finding_ids),
                con,
            )
        else:
