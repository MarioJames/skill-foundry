...
def _validate_constraints(parent, child):
    ...
    if "profile_hint" in child and (
        not isinstance(child["profile_hint"], str)
        or not child["profile_hint"].strip()
    ):
        _error("constraints.profile_hint must be a non-empty profile name")
...
def _create_tasks(
    ...
    *,
    ...
    if prior_children and count_replan:
...
def _mode_task_compiler(con, context, action_id):
    def compile_tasks(specs):
        try:
...
def _start_mode(con, context, payload, action_id):
    try:
        response = mode_runtime.start_mode(
            con,
            context,
            payload,
            action_id,
            _mode_task_compiler(con, context, action_id),
        )
    except ValueError as exc:
        _error(str(exc))
    schedule_required = response.pop("schedule_required", False)
    if schedule_required:
        scheduler.schedule_with_connection(con, context["run"]["root_id"])
    return response


def _advance_mode(con, context, payload, action_id):
    try:
        response = mode_runtime.advance_mode(
            con,
            context,
            payload,
            action_id,
            _mode_task_compiler(con, context, action_id),
            recovery.cancel_mode_with_connection,
        )
    except ValueError as exc:
        _error(str(exc))
    schedule_required = response.pop("schedule_required", False)
...
def _validate_done_payload(con, context, payload):
    ...
    try:
        mode_runtime.validate_task_mode_result(con, context, payload)
        mode_runtime.validate_owner_modes_finished(con, context["task"]["task_id"])
    except ValueError as exc:
        _error(str(exc))
    ...
    if isinstance(review, dict):
        ...
        if source is not None:
            if source == "self":
                if context["task"].get("resolved_intent") != "review":
                    _error("review source self is valid only for the current review task")
            else:
                if isinstance(source, bool) or not isinstance(source, int):
                    _error("review source must be self or an integer task_id")
...
def _finish(con, context, payload, action_id):
    ...
    for field in ("validation", "review", "integration_check", "mode_result"):
    ...
    if status == "done":
        ...
        if changed_files and (
            ...
            or task.get("resolved_intent") == "review"
        ):
            _error("read-only or review tasks cannot finish done with changed_files")
...
def _process_action(envelope, poll_interval=0.1):
    ...
    with state_store.transaction() as con:
        ...
        if cached is not None:
            ...
            elif action_type == "start_mode":
                response = _start_mode(con, context, payload, envelope["action_id"])
            elif action_type == "advance_mode":
                response = _advance_mode(con, context, payload, envelope["action_id"])
