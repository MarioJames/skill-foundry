...
def _spawn(effect, payload, adapter=None):
    with state_store.transaction(immediate=False) as con:
        ...
        boundary_values = {
            ...
            "HOME": str(state_store.runtime_root()),
            "SKILL_DIR": str(pathlib.Path(__file__).resolve().parent.parent),
        }
        env = compat_env.export_both(
            boundary_values,
            scrub_identity=True,
        )
    ...
    # ACP's compatibility Worker still derives its subprocess environment from
    # os.environ.  Temporarily binding the exact child identity prevents a
    # canonical parent identity leaking beside the frozen legacy child identity.
    with compat_env.process_boundary(boundary_values):
