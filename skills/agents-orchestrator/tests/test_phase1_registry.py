...
class Phase1RegistryTests(unittest.TestCase):
    ...
    def test_default_dependency_set_installs_codex_and_claude_profiles(self):
        with mock.patch.object(
            registry,
            "install_profile",
            side_effect=lambda profile, environment=None: profile,
        ) as installer:
            installed = registry.install_default_profiles(environment={"PATH": ""})

        self.assertEqual({"codex", "claude"}, set(installed))
        self.assertEqual(
            ["codex", "claude"],
            [call.args[0]["agent"] for call in installer.call_args_list],
        )
    ...
    def test_registry_preflight_reports_managed_sdk_without_installing(self):
        ...
        self.assertEqual("managed", report["sdk"]["source"])
        self.assertTrue(report["sdk"]["available"])
        self.assertEqual("agent-client-protocol==0.11.0", report["sdk"]["requirement"])
