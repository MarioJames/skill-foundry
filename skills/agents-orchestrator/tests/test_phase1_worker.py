...
class Phase1WorkerTests(unittest.TestCase):
    ...
    def test_fake_acp_child_finishes_and_persists_real_session_id(self):
        with isolated_runtime() as (runtime_home, cwd):
            ...
            try:
                ...
                self.assertEqual(1, drained["completed"], drained)
    ...
    def test_acp_worker_injects_persistent_mode_context_into_real_prompt(self):
        with isolated_runtime() as (_, cwd):
            prompt_file = cwd / "captured-mode-prompt.txt"
            identity = agent_orchestrator.initialize_run(
                "mode prompt root",
                str(cwd),
                backend="acp",
            ...
            def invoke(action_type, payload, action_id):
            ...
            try:
                invoke(
                    "submit_estimate",
                    {
                        ...
                        "unknowns": [],
                        "estimated_files": [],
                        "reason": "exercise the detached ACP prompt path",
                    },
                    "estimate-mode-prompt",
                )
                started = invoke(
                    "start_mode",
                    {
                        "mode": "swarm",
                        "objective": "Verify real Worker prompt propagation.",
                        "evidence": {"content": "WORKER_MODE_CONTEXT_SENTINEL"},
                        "tasks": [
                            {
                                "key": "prompt-probe",
                                "goal": "Hold after receiving the prompt.",
                                "intent_hint": "review",
                                "output_contract": "Expose the received prompt to the test fixture.",
                            }
                        ],
                    },
                    "start-mode-prompt",
                )
                self.assertEqual(1, len(started["task_ids"]))
                drained = outbox.drain(identity["root_id"])
                self.assertEqual(1, drained["completed"], drained)
                wait_for(prompt_file.exists)
                prompt = prompt_file.read_text(encoding="utf-8")
                self.assertIn("[MODE CONTEXT]", prompt)
                self.assertIn("dependency_evidence_bundle", prompt)
                self.assertIn("WORKER_MODE_CONTEXT_SENTINEL", prompt)
            finally:
                self._stop(identity)
