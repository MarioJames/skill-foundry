...
安装器／清理器运行后应删除指定文件并保留用户文件，也是运行时契约，区别于断言仓库里某个旧文件不存在。明确要求旧协议返回指定错误时，从协议入口验证结果，不为删除任务自行新增拒绝分支。也不要用纪念性注释、`skip` / `todo` 或兼容开关保留被删除功能的痕迹。
...
| 新增文件或导出 | 只断言文件存在、符号导出、组件能 mount | 用类型检查、导入解析、lint 和已有消费者测试确认结构；有接线风险时验证实际操作 |
...
| 使用标准库或第三方库 | 重复验证依赖自己的行为，没执行项目代码 | 验证项目对依赖的使用与结果处理；请求参数、幂等调用次数确属契约时可以断言交互 |
| 修改一处运营文案 | 新增整页 DOM 快照，或为删除提示新增“不再出现”用例 | 更新受影响的现有 locator／断言并做浏览器验收 |
| 新增快照 | 把刚跑出的输出或被测配置直接当答案 | 按独立需求核对预期，只固定有意义的结果；简单业务事实可以硬编码 |
...
| 让实现更容易测试 | 为凑用例增加导出、抽象、注入开关、fallback 或防御分支，再测这些结构 | 缩小被测边界，复用已有测试接缝，不让测试为额外业务代码找理由 |
...
测试命名说清行为，便于定位失败；不要按是否出现 `and`、用了哪种断言 API 或有几个断言机械判定质量。优先覆盖可组合的叶子规则；风险来自交互时保留有意义的组合用例。
...
用一个具体错误检查断言价值：角色映射错误或接受负金额应导致以上断言失败。这个思考不要求引入 mutation testing 工具或固定流程。
