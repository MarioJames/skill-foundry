...
const RUNTIME_SOURCE = join(SKILL_ROOT, "assets", "server-runtime.zsh");
const roots: string[] = [];
  ...
  test("rejects invalid profile fields instead of silently normalizing them", () => {
    ...
    const result = Bun.spawnSync({
      cmd: [
        ...
        "-c",
        `source "$HOME/.config/zsh/server-runtime.zsh"
server_define BadName ubuntu example.invalid 22 '' >/dev/null 2>&1; print -r -- "name:$?"
server_define validname 'bad!user' example.invalid 22 '' >/dev/null 2>&1; print -r -- "user:$?"
server_define validname ubuntu -badhost 22 '' >/dev/null 2>&1; print -r -- "host:$?"
server_define validname ubuntu example.invalid 65536 '' >/dev/null 2>&1; print -r -- "port:$?"`,
      ],
      env,
      stdin: "ignore",
      stdout: "pipe",
  ...
  test("normalizes decimal ports and brackets bare IPv6 hosts only in links", () => {
    const { env } = fixture();
    const result = Bun.spawnSync({
      cmd: [
        "zsh",
        ...
        "-c",
        `source "$HOME/.config/zsh/server-runtime.zsh"
server_define ipv6host ubuntu 2001:db8::20 0022 ''
server_link ipv6host
print -r -- "$IPV6HOST_IP|$IPV6HOST_PORT"`,
      ],
      env,
      stdin: "ignore",
      stdout: "pipe",
