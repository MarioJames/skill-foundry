...
def _resolve(explicit, env_name, label, required=True):
    environment = os.environ.get(env_name, "").strip() or None
    if explicit and environment and explicit != environment:
        raise ValueError("explicit %s does not match %s" % (label, env_name))
    value = explicit or environment
    if required and not value:
        raise ValueError("%s is required (argument or %s)" % (label, env_name))
    return value
